<template>
    ...
    <div class="card mb-3">
        ...
        <div class="card-footer text-end">
            <button class="btn btn-success me-2" @click="updateQuestion()">Update</button>
        </div>
    </div>
    <!-- Choices -->
    <div class="card mb-3">
        ...
        <div class="card-footer text-end">
            <button class="btn btn-success me-2" @click="updateChoices()">Update</button>
        </div>
    </div>
    <!-- Solution -->
    <div class="card">
        <div class="card-header">
            <h6>Edit Soultions</h6>
        </div>
        <div class="card-body">
            <section class="mb-3">
                <div class="row mb-3">
        ...
        <div class="card-footer text-end">
            <button class="btn btn-success me-2" @click="updateSolution()">Update</button>
        </div>
    </div>
...
export default {
    ...
    setup() {
        ...
        const setQuestionDetails = () => {
            var tapasyaAdminToken = localStorage.getItem("tapasya_admin_token");
            axios.get(BASE_URL+'admin/question-bank/'+route.params.id, {
                headers: {
                    Authorization: 'Bearer ' + tapasyaAdminToken
                }
            })
            .then(async function(response) {
                ...
                questionData.value = response.data.content;
                grade.value = response.data.grade_id;
                subject.value = response.data.subject_id;
                await loadChapters();
                chapter.value = response.data.chapter_id;
                difficultyLevel.value = response.data.difficulty_level;
                choices.value = response.data.choices;
                solutionData.value = response.data.solution.content;
                solutionVideo.value = response.data.solution.video;
            })
            .catch(function(error) {
                notice_message.value = 'Something went wrong';
				notice_toast.value.show();
        ...
        const updateQuestion = () => {
            question.value.grade = grade.value;
            question.value.subject = subject.value;
            question.value.chapter = chapter.value;
            question.value.difficulty_level = difficultyLevel.value;
            question.value.content = questionData.value;

            loading_modal.value.show();
            var tapasyaAdminToken = localStorage.getItem("tapasya_admin_token");           
            axios.post(BASE_URL + 'admin/question/' + question.value.id, {
                _method: 'PUT',
        ...
        const updateChoices = () => {
            loading_modal.value.show();
            var tapasyaAdminToken = localStorage.getItem("tapasya_admin_token");
            axios.post(BASE_URL + 'admin/choice/' + question.value.id, {
                _method: 'PUT',
        ...
        const updateSolution = () => {
            loading_modal.value.show();
            var tapasyaAdminToken = localStorage.getItem("tapasya_admin_token");
            axios.post(BASE_URL+'admin/solution/'+question.value.id, {
                _method: 'PUT',
                solution: {
