	...
	<div>
		...
		<div class="card">
						...
						<div class="card mt-3">
							<div class="card-body">
								<div class="mb-3">
									...
									<input id="examName" type="text" v-model="examName" class="form-control" :class="{'is-invalid': errors['name']}" placeholder="Enter Exam Name" />
								...
								<hr />
								<div v-for="(syllabusItem, index) in syllabus">
			...
			<div class="card-footer text-center">
				<button class="btn btn-primary me-2" @click="storeDailyTestSeries()">Submit</button>
				<router-link to="/daily-test-series/create" class="btn btn-danger text-light">Discard</router-link>
			</div>
		</div>
		<!-- Toast Component -->
		...
		const loadAllSubjects = () => {
			...
			axios.get(BASE_URL+'admin/configure/subject', {
				...
				.then(function(response) {
					...
					syllabus.value.forEach((syllabusItem) => syllabusItem.count = 0)
